                ...
                _ = shutdown_ch.recv() => {
                        ...
                        while retries_in_channel {
                            ...
                            if bat_w_req.len() > 0 {
                                ...
                                // check if last persist generated unprocessed items - maybe remove??
                                tokio::select! {
                                    biased;
                            ...
                            loop {
                                ...
                                for item in items.unwrap() {
                                    ...
                                    for hm in item {
                                        ...
                                        if bat_w_req.len() == DYNAMO_BAT_SIZE {
                                            println!("load_task: persist retry batch...");
        ...
        Ok(resp) => {
            if resp.unprocessed_items.as_ref().unwrap().values().len() > 0 {
                ...
                for (k, v) in resp.unprocessed_items.unwrap() {
                    println!("retry: persist_batch, unprocessed items..delay 5secs");
                    sleep(Duration::from_millis(5000)).await;
