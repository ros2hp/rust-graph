    ...
    loop {
        ...
        if batch.is_err() {
            ...
            rt_handle.block_on(async move {
                ...
                if bat_w_req.len() > 0 {
                    persist_dynamo_batch(dynamo_client, bat_w_req, &retry_ch).await;
                }
            });
            break;
        }
        ...
        for node in batch.unwrap().into_iter() {
            ...
            bat_w_req = rt_handle.block_on(async {   // no move. bat_w_req moved automatically as it is moved in one of the dynamo API's
                ...
                if bat_w_req.len() == DYNAMO_BAT_SIZE {
                    bat_w_req = persist_dynamo_batch(dynamo_client, bat_w_req, &retry_ch).await;
                }
                ...
                for (k,nv) in flat_rdf_map {
                    ...
                    // this will probably involve more than one node's worth of put_items
                    if bat_w_req.len() == DYNAMO_BAT_SIZE {
                         bat_w_req = persist_dynamo_batch(dynamo_client, bat_w_req, &retry_ch).await;
                    }
                }
